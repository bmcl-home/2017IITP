...
class Pool():
	...
	def put(self, x):
		...
		self.data_a [self.idx:self.idx+size] = torch.from_numpy(a).view([-1,1])
		self.data_r [self.idx:self.idx+size] = torch.from_numpy(r).view([-1,1])
		self.data_s_[self.idx:self.idx+size] = torch.from_numpy(s_)

		self.idx = (self.idx + size) % self.size

	def sample(self, size):
		idx = np.array(torch.from_numpy(np.random.choice(self.size, size)).cuda())
